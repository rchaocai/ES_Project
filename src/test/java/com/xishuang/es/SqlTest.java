...
//        String sql = "select host.keyword, count() as count, sum(bytes) from kibana_sample_data_logs group by host having count > 280 order by count limit 5";
        String sql = "select * from (select host,bytes,response,row_number()over(partition by host order by bytes desc) rank from kibana_sample_data_logs limit 3) where rank <1";
